      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="logo-on-dark inline-block mb-4">
              <img src={companyInfo.logo} alt="Gurkhas Cleaning Services" className="h-12 w-auto object-contain rounded-lg" style={{ maxWidth: '150px' }} />
            </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {navigation.map((item) => (
                <li key={item.name}>
                  <Link to={item.href} className="text-white/50 hover:text-[#f59e0b] text-sm transition-colors">{item.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              {['Commercial Cleaning', 'Office Cleaning', 'End of Lease', 'Window Cleaning', 'Carpet Cleaning', 'Residential Cleaning'].map((s) => {
                ...
                return (
                  <li key={s}>
                    <Link to={`/services/${slug}`} className="text-white/50 hover:text-[#f59e0b] transition-colors">{s}</Link>
                  </li>
                );
              })}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li>
                <a href={`tel:${companyInfo.phoneRaw}`} className="flex items-start gap-3 hover:text-[#f59e0b] transition-colors">
                  <Phone size={17} className="text-[#2563eb] flex-shrink-0 mt-0.5" />
                  <span className="font-semibold text-sm">{companyInfo.phone}</span>
                </a>
              </li>
              <li>
                <a href={`mailto:${companyInfo.email}`} className="flex items-start gap-3 text-white/50 hover:text-[#f59e0b] transition-colors">
                  <Mail size={17} className="text-[#2563eb] flex-shrink-0 mt-0.5" />
                  <span className="text-sm break-all">{companyInfo.email}</span>
                </a>
              </li>
              <li>
                <div className="flex items-start gap-3 text-white/50">
                  <MapPin size={17} className="text-[#2563eb] flex-shrink-0 mt-0.5" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <div className="flex gap-5">
            {footerLinks.map((link) => (
              <Link key={link.name} to={link.href} className="text-white/35 hover:text-white text-xs transition-colors">{link.name}</Link>
            ))}
          </div>
        </div>
      </div>

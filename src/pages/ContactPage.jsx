      ...
      <main>
        <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#0f1a2e] via-[#1a2744] to-[#1e3060] relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto text-center relative z-10">
            <span className="inline-block bg-[#2563eb]/20 text-blue-300 px-4 py-1.5 rounded-full text-sm font-bold mb-4 tracking-wide uppercase">Get In Touch</span>
            <h1 className="font-display text-4xl sm:text-5xl font-extrabold text-white mb-5">Contact Us</h1>
            <p className="text-white/65 text-lg max-w-2xl mx-auto">Have questions? We'd love to hear from you — reach out and we'll respond promptly.</p>
          </div>
        </section>
                ...
                {contactInfo.map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="w-12 h-12 bg-[#eff6ff] rounded-xl flex items-center justify-center flex-shrink-0">
                      <item.icon className="text-[#2563eb]" size={21} />
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1 font-medium uppercase tracking-wide">{item.label}</p>
                      {item.href ? (
                        <a href={item.href} className="text-gray-900 font-semibold hover:text-[#2563eb] transition-colors text-sm">{item.value}</a>
                      ) : (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        ...
                        <Input id="name" name="name" value={formData.name} onChange={handleChange} required className="mt-2 h-12 rounded-xl border-gray-200 focus:border-[#2563eb]" />
                      </div>
                      <div>

...
const serviceDetails = {
  ...
  'office-cleaning': {
    ...
    why: 'Clean offices reduce sick days, improve focus and create a positive first impression for every visitor or client.',
  },
  'window-cleaning': {
    ...
    why: 'Clean windows let in more natural light, improving the look and feel of any space. We use professional equipment for streak-free results.',
  },
  'carpet-cleaning': {
    tagline: 'Deep clean that brings your carpets back to life.',
    includes: ['Hot water extraction (steam cleaning)','Pre-treatment of stains and high-traffic areas','Deodorising and sanitising treatment','Suitable for all carpet types and fibres','Dries within 2–4 hours','Removes allergens, dust mites and bacteria'],
  ...
  'residential-cleaning': {
    ...
    why: 'Let us handle the cleaning so you can spend time on what matters. Consistent, reliable, and tailored to how you like your home.',
  },
};
...
const ServiceDetailPage = () => {
  ...
  if (!service || !detail) return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-4 pt-32">
        ...
        <Link to="/services" className="text-[#166534] font-semibold hover:underline">← Back to Services</Link>
      </main>
      <Footer />
    </div>
  );
  ...
  return (
    <div className="min-h-screen">
      ...
      <main>
        {/* Hero */}
        <section className="relative pt-32 pb-24 px-4 sm:px-6 lg:px-8 bg-cover bg-center overflow-hidden"
          style={{ backgroundImage:`url(${service.image})` }}>
          <div className="absolute inset-0 bg-gradient-to-br from-[#14532d]/92 via-[#166534]/88 to-[#15803d]/80" />
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute -top-20 -right-20 w-72 h-72 rounded-full bg-[#84cc16]/10 blur-3xl" />
          </div>
          <div className="relative z-10 max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="animate-fade-up w-14 h-14 bg-[#84cc16] rounded-2xl flex items-center justify-center shadow-md">
                <Icon className="text-[#14532d]" size={26} />
              </div>
              <span className="animate-fade-up inline-block bg-[#84cc16]/20 text-[#bef264] px-4 py-1.5 rounded-full text-sm font-bold tracking-wide uppercase">
                Our Services
              </span>
            </div>
            ...
            <div className="animate-fade-up delay-300 flex flex-col sm:flex-row gap-4">
              <Link to={`/quotes?service=${encodeURIComponent(service.name)}`}
                className="inline-flex items-center justify-center gap-2 bg-[#84cc16] text-[#14532d] px-8 py-4 rounded-full font-extrabold hover:bg-[#65a30d] hover:text-white transition-all hover:scale-105 shadow-md shadow-black/20">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <div className="lg:col-span-2 space-y-8">
                <div className="animate-fade-up bg-white rounded-2xl p-8 shadow-sm border border-[#dcfce7]">
                ...
                <div className="animate-fade-up delay-100 bg-white rounded-2xl p-8 shadow-sm border border-[#dcfce7]">
              ...
              <div className="space-y-6">
                <div className="animate-fade-up bg-[#166534] rounded-2xl p-7 text-white">
                  ...
                  <Link to={`/quotes?service=${encodeURIComponent(service.name)}`}
                    className="block text-center bg-[#84cc16] text-[#14532d] px-6 py-3.5 rounded-full font-extrabold hover:bg-[#65a30d] hover:text-white transition-colors mb-3">
                    Get a Free Quote
                  </Link>
                  <a href={`tel:${companyInfo.phoneRaw}`}
                ...
                <div className="animate-fade-up delay-100 bg-white rounded-2xl p-7 shadow-sm border border-[#dcfce7]">
                    ...
                    {otherServices.map((s) => {
                      ...
                      return (
                        <Link key={s.id} to={`/services/${toSlug(s.name)}`}
                          className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f7fee7] transition-colors group">
                          <div className="w-9 h-9 rounded-lg bg-[#dcfce7] group-hover:bg-[#84cc16] flex items-center justify-center transition-colors flex-shrink-0">
                            <OtherIcon size={16} className="text-[#166534] group-hover:text-[#14532d] transition-colors" />
                          </div>
                          <span className="text-gray-700 font-medium text-sm group-hover:text-[#166534] transition-colors">{s.name}</span>
                          <ArrowRight size={14} className="text-gray-300 group-hover:text-[#166534] ml-auto transition-colors" />
                        </Link>
                      );
                    })}
                    <Link to="/services"
                      className="flex items-center gap-1.5 text-[#166534] font-semibold text-sm pt-2 hover:gap-3 transition-all">
            ...
            <div className="mt-12">
              <button onClick={() => navigate(-1)}
                className="inline-flex items-center gap-2 text-gray-400 hover:text-[#166534] text-sm font-medium transition-colors">
